...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                w_pos_emb=nn.Embedding(
                    config.block_size, config.n_embd
                ),  # in this assignment, you have to instead use the rotary positional embeddings, but we keep the placeholder if you want to use the pre-trained model
                drop=nn.Dropout(config.embd_pdrop),
                h=nn.ModuleList(
                    [TransformerDecoderBlock(config) for _ in range(config.n_layer)]
                ),
    ...
    def from_pretrained(cls, model_type):
        ...
        assert model_type in {
            ...
            "gpt2-large",
            "gpt2-xl",
        }, "No pretrained weights available for specified model-type.. Choose between 'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'"
